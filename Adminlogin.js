...
export default function Adminlogin() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:8081/Admin/Login', { name,email, password });
      const result = response.data;
      if (result === 'Admin Successfully') {
        // Admin successfully logged in, store login status and navigate to AdminDashboard
