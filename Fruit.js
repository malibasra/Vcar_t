...
export default function Fruit() {
  ...
  const handleAddToCart = (index, product) => {
    ...
    if (loginStatus === 'Login Successfully' || loginStatus === 'Admin Successfully') {
  ...
  return (
    <div>
      ...
      <div className="fst-italic text-center bg-primary-subtle vh-150">
        <h1 style={{ fontSize: '100px' }}>Fruits</h1>
        <div className="container">
          <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-3 justify-content-center">
            {products.filter(product => product.ProductCategoryFID === 4 && product.ProductSize > 0).length > 0 ? (
              products.filter(product => product.ProductCategoryFID === 4 && product.ProductSize > 0).map((product, index) => (
                <div key={product.ProductID} className="col">
                  <div className="card h-100">
                    ...
                    <div className="card-body d-flex flex-column">
                      ...
                      <p className="card-title" style={{ fontSize: '12px' }}>{product.ProductPrice}</p>
